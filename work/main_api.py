...
class waAPI(object):
    ...
    def spoken_results(self, key=None, i=None):
        """
        ...
        if i is None:
            ...
            url = '%s?i=%s&appid=%s' % (SR_ROOT, i.lower(), key)
            print(url)
            r = requests.get(url)
    ...
    def short_answers(self, key=None, i=None):
        """
        Calls the API and returns a string of text
        ...
        if i is None:
            ...
            url = '%s?i=%s&appid=%s' % (SA_ROOT, i.encode('utf8').lower(), key)
            r = requests.get(url)
    ...
    def full_results(self, response_format=None, key=None, i=None):
        """
        Calls the API and returns a dictionary of the search results
